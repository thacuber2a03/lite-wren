...
const char *system_source = "class Program {\n"
                            ...
                            "\n"
                            "    static executableDirectory {\n"
                            "        if (!__executableDirectory) {\n"
                            "            var exename = Program.executablePath\n"
                            "            for (i in (exename.count-1)..0) {\n"
                            "                if (exename[i] == \"/\") {\n"
                            "                    __executableDirectory = exename[0...i]\n"
                            "                    return __executableDirectory\n"
                            "                }\n"
                            "            }\n"
                            "            __executableDirectory = \"\"\n"
                            "        }\n"
                            "        return __executableDirectory\n"
                            "    }\n"
                            "\n"
...
const char *renderer_source =
    ...
    "        if ((c is List) && (c.count == 3 || c.count == 4)) return c\n"
    "        Fiber.abort(\"expected a list of length 3 or 4 for a color, got %(c)\")\n"
    "    }\n"
    "\n"
    "    static drawText(font, text, x, y, color) { drawText_(font, text.toString, x, y, checkColor_(color)) }\n"
    "    static drawText(font, text, x, y) { drawText_(font, text.toString, x, y, List.filled(4,255)) }\n"
